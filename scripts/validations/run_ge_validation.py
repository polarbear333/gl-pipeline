...
DATA_ASSET_NAME = "stg_ledger"
DATA_DIR = "data/silver"
DATA_DOCS_DIR = "great_expectations/uncommitted/data_docs/local_site"
...
def build_validator(context: Any, asset: Any):
    ...
    # Column existence
    validator.expect_column_to_exist("ledger_sk")
    validator.expect_column_to_exist("amount")
    validator.expect_column_to_exist("fiscal_year")
    validator.expect_column_to_exist("ledger")

    # Not nulls
    validator.expect_column_values_to_not_be_null("ledger_sk")
    validator.expect_column_values_to_not_be_null("amount")

    # Uniqueness
    validator.expect_column_values_to_be_unique("ledger_sk")
    ...
    validator.expect_column_values_to_be_in_set(
        "ledger", value_set=["ACTUALS", "BUDGET", "ENCUMBRANCE", "APPROPRIATION"]
    )

    validator.expect_column_min_to_be_between(
        "amount", 
        min_value=-20_000_000_000.00, 
        strict_min=False 
    )
    validator.expect_column_max_to_be_between(
        "amount",
        max_value=20_000_000_000.00, 
        strict_max=False
    )
...
def parse_and_log_failures(result: Any):
    ...
    for i, failure in enumerate(failed, 1):
        ...
        # GE 1.x uses 'type' in ExpectationConfiguration
